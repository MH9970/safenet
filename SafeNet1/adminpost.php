...
// Default log_id for anonymous user since login check removed
$log_id = 0;
...
// AJAX: Handle reactions
if (isset($_POST['react_post'])) {
    $post_id = $_POST['post_id'];
    $type = $_POST['type']; // 'like' or 'love'
    ...
    if ($type === 'like') {
        if(in_array($log_id, $likes)) { // toggle off
            $likes = array_diff($likes, [$log_id]);
        } else {
            $likes[] = $log_id;
        }
    }
    ...
    if ($type === 'love') {
        if(in_array($log_id, $loves)) {
            $loves = array_diff($loves, [$log_id]);
        } else {
            $loves[] = $log_id;
        }
    }
...
// AJAX: Handle comment submission
if (isset($_POST['submit_comment'])) {
    ...
    // Fetch latest comment with user info (if log_id=0, use placeholder)
    if($log_id == 0){
        $u = ['f_name'=>'Anonymous','l_name'=>'','photo'=>'default.png'];
    } else {
...
$query = "SELECT p.*, l.f_name, l.l_name, l.photo AS user_photo 
          FROM post p LEFT JOIN log_in l ON p.log_id = l.log_id ORDER BY p.post_id DESC";
...
.post-description{ width:100%; border:none; resize:none; outline:none; background:transparent; font-size:16px; margin-top:10px; line-height:1.5; white-space:pre-wrap; }
    ...
    <div id="posts">
        ...
        <div class="post" data-postid="<?= $row['post_id'] ?>">
            <div class="post-header">
                <img src="uploads/<?= $row['user_photo'] ?: 'default.png' ?>" class="user-photo">
                <div>
                    <h3><?= $row['f_name'].' '.$row['l_name'] ?: 'Anonymous' ?></h3>
            ...
            <div class="comment-section">
                <div class="comment-box">
                    <?php foreach($comments as $c):
                        $u = $c['user_id'] == 0 ? ['f_name'=>'Anonymous','l_name'=>'','photo'=>'default.png'] : mysqli_fetch_assoc(mysqli_query($con, "SELECT f_name,l_name,photo FROM log_in WHERE log_id=".$c['user_id']));
                    ?>
...
document.querySelectorAll('.comment-form').forEach(form=>{
    form.addEventListener('submit', function(e){
        ...
        .then(data=>{
            const box = this.previousElementSibling; // comment-box
